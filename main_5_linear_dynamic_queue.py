"""
Main script for Linear Dynamic Queue training and evaluation.
Supports: LinearDynamicQueueDQN, LinearDynamicQueueDDQN, LinearDynamicQueueDueling, LinearDynamicQueuePerDQN, LinearDynamicQueueRainbow, LinearDynamicQueueMultiStepDQN
...
from agents.linear_dynamic_queue_dqn_agent import LinearDynamicQueueDQNAgent
from agents.linear_dynamic_queue_ddqn_agent import LinearDynamicQueueDDQNAgent
from agents.linear_dynamic_queue_dueling_agent import LinearDynamicQueueDuelingAgent
from agents.linear_dynamic_queue_per_agent import LinearDynamicQueuePerAgent
from agents.linear_dynamic_queue_rainbow_agent import LinearDynamicQueueRainbowAgent
from agents.linear_dynamic_queue_multistep_agent import LinearDynamicQueueMultiStepAgent
from agents.linear_dynamic_queue_fcfs_agent import LinearDynamicQueueFCFSAgent
from agents.linear_dynamic_queue_greedy_agent import LinearDynamicQueueGreedyAgent
from agents.dqn_agent import DQNAgent
from common.linear_dynamic_queue_env import LinearDynamicQueueEnv
...
from simulation.linear_dynamic_queue_simulation import run_linear_dynamic_queue_simulation
...
def get_agent(algo_name: str, seed: int = 0):
    if algo_name == "LinearDynamicQueueDQN":
        return LinearDynamicQueueDQNAgent(STATE_SIZE_DYNAMIC_QUEUE, ACTION_SIZE_DYNAMIC_QUEUE, seed=seed)
    if algo_name == "LinearDynamicQueueDDQN":
        return LinearDynamicQueueDDQNAgent(STATE_SIZE_DYNAMIC_QUEUE, ACTION_SIZE_DYNAMIC_QUEUE, seed=seed)
    if algo_name == "LinearDynamicQueueDueling":
        return LinearDynamicQueueDuelingAgent(STATE_SIZE_DYNAMIC_QUEUE, ACTION_SIZE_DYNAMIC_QUEUE, seed=seed)
    if algo_name == "LinearDynamicQueuePerDQN":
        return LinearDynamicQueuePerAgent(STATE_SIZE_DYNAMIC_QUEUE, ACTION_SIZE_DYNAMIC_QUEUE, seed=seed)
    if algo_name == "LinearDynamicQueueRainbow":
        return LinearDynamicQueueRainbowAgent(STATE_SIZE_DYNAMIC_QUEUE, ACTION_SIZE_DYNAMIC_QUEUE, seed=seed)
    if algo_name == "LinearDynamicQueueMultiStepDQN":
        return LinearDynamicQueueMultiStepAgent(STATE_SIZE_DYNAMIC_QUEUE, ACTION_SIZE_DYNAMIC_QUEUE, seed=seed)
    if algo_name == "LinearDynamicQueueFCFS":
        return LinearDynamicQueueFCFSAgent(STATE_SIZE_DYNAMIC_QUEUE, ACTION_SIZE_DYNAMIC_QUEUE, seed=seed)
    if algo_name == "LinearDynamicQueueGreedy":
        return LinearDynamicQueueGreedyAgent(STATE_SIZE_DYNAMIC_QUEUE, ACTION_SIZE_DYNAMIC_QUEUE, seed=seed)
    if algo_name == "LinearDynamicQueueFORLAPS":
...
def train_linear_dynamic_queue_generic(agent, algo_name: str, num_patients: int, gen_id: int, train_config: dict):
    ...
    env = LinearDynamicQueueEnv(STATE_SIZE_DYNAMIC_QUEUE, ACTION_SIZE_DYNAMIC_QUEUE, data_path=config["data_file"])
...
SUPPORTED_ALGOS = [
    "LinearDynamicQueueDQN", "LinearDynamicQueueDDQN", "LinearDynamicQueueDueling", 
    "LinearDynamicQueuePerDQN", "LinearDynamicQueueRainbow", "LinearDynamicQueueMultiStepDQN",
    "LinearDynamicQueueFCFS", "LinearDynamicQueueGreedy", "LinearDynamicQueueFORLAPS"
]

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("❌ Missing algorithms")
        print("   Example: python main_5_linear_dynamic_queue.py LinearDynamicQueueDQN 200")
    ...
    # Baseline algorithms
    if ALGO_TO_RUN in ["LinearDynamicQueueFCFS", "LinearDynamicQueueGreedy", "LinearDynamicQueueFORLAPS"]:
        ...
        baseline_avg_time = run_linear_dynamic_queue_simulation(NUM_PATIENTS, None, "random_base_eval", EVAL_SEED, "Random", 0)
        _append_note(log_dir, f"Random base Avg Time: {baseline_avg_time:.2f}\n", NUM_PATIENTS)
        
        avg_time = run_linear_dynamic_queue_simulation(NUM_PATIENTS, agent, f"linear_{ALGO_TO_RUN.lower()}", EVAL_SEED, ALGO_TO_RUN, 0)
    ...
    if os.path.exists(random_base_queue_log):
        print(f"✅ Found existing Linear Dynamic Queue random_base")
        baseline_avg_time = run_linear_dynamic_queue_simulation(NUM_PATIENTS, None, "random_base_eval", EVAL_SEED, "Random", 0)
    else:
        print(f"⚠️ Linear Dynamic Queue random_base not found. Generating...")
        baseline_avg_time = run_linear_dynamic_queue_simulation(NUM_PATIENTS, None, "random_base", EVAL_SEED, "Random", 0)
    ...
    for gen_id in range(1, 4):
        ...
        ckpt_paths = train_linear_dynamic_queue_generic(agent, ALGO_TO_RUN, NUM_PATIENTS, gen_id, train_config)
        ...
        for ckpt_path in ckpt_paths:
            ...
            avg_time = run_linear_dynamic_queue_simulation(NUM_PATIENTS, agent_eval, f"LinearDynamicQueue_g{gen_id}_ep{ep}", EVAL_SEED, ALGO_TO_RUN, gen_id)
            improvement = ((baseline_avg_time - avg_time) / baseline_avg_time) * 100 if baseline_avg_time > 0 else 0.0
        ...
        if best_ckpt:
            ...
            if gen_id < 3 and (gen_id + 1) in train_config:
                ...
                run_linear_dynamic_queue_simulation(NUM_PATIENTS, agent_for_data, f"LinearDynamicQueue_gen_{gen_id}", EVAL_SEED, ALGO_TO_RUN, gen_id)
