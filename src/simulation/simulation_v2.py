...
# -----------------------------
def run_simulation(num_patients=50, mode="greedy-epsilon"):
    """
    Chạy mô phỏng với số bệnh nhân num_patients
    mode: "greedy-epsilon", "dqn", "dbcq"
    """
    env = simpy.Environment()
    center = HealthCheckCenter(env)
    coordinator = NextBestAction(mode, epsilon=0.3)  # epsilon chỉ áp dụng cho greedy-epsilon
    all_patients = []
    ...
    out_file = os.path.join(output_dir, f"{mode}_event_log.csv")
...
if __name__ == "__main__":
    df, metrics = run_simulation(20, "dqn")