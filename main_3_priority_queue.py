"""
Main script for Priority Queue training and evaluation.
Supports: PriorityQueueDQN, PriorityQueueDDQN, PriorityQueueDueling, PriorityQueuePerDQN, PriorityQueueRainbow, PriorityQueueMultiStepDQN
...
from agents.priority_queue_fcfs_agent import PriorityQueueFCFSAgent
from agents.priority_queue_greedy_agent import PriorityQueueGreedyAgent
...
def get_agent(algo_name: str, seed: int = 0):
    if algo_name == "PriorityQueueDQN":
        return PriorityQueueDQNAgent(STATE_SIZE_PRIORITY_QUEUE, ACTION_SIZE_PRIORITY_QUEUE, seed=seed)
    if algo_name == "PriorityQueueDDQN":
        return PriorityQueueDDQNAgent(STATE_SIZE_PRIORITY_QUEUE, ACTION_SIZE_PRIORITY_QUEUE, seed=seed)
    if algo_name == "PriorityQueueDueling":
        return PriorityQueueDuelingAgent(STATE_SIZE_PRIORITY_QUEUE, ACTION_SIZE_PRIORITY_QUEUE, seed=seed)
    if algo_name == "PriorityQueuePerDQN":
        return PriorityQueuePerAgent(STATE_SIZE_PRIORITY_QUEUE, ACTION_SIZE_PRIORITY_QUEUE, seed=seed)
    if algo_name == "PriorityQueueRainbow":
        return PriorityQueueRainbowAgent(STATE_SIZE_PRIORITY_QUEUE, ACTION_SIZE_PRIORITY_QUEUE, seed=seed)
    if algo_name == "PriorityQueueMultiStepDQN":
        return PriorityQueueMultiStepAgent(STATE_SIZE_PRIORITY_QUEUE, ACTION_SIZE_PRIORITY_QUEUE, seed=seed)
    if algo_name == "PriorityQueueFCFS":
        return PriorityQueueFCFSAgent(STATE_SIZE_PRIORITY_QUEUE, ACTION_SIZE_PRIORITY_QUEUE, seed=seed)
    if algo_name == "PriorityQueueGreedy":
        return PriorityQueueGreedyAgent(STATE_SIZE_PRIORITY_QUEUE, ACTION_SIZE_PRIORITY_QUEUE, seed=seed)
    if algo_name == "PriorityQueueFORLAPS":
        return DQNAgent(STATE_SIZE_PRIORITY_QUEUE, ACTION_SIZE_PRIORITY_QUEUE, seed=seed)
...
def train_priority_queue_generic(agent, algo_name: str, num_patients: int, gen_id: int, train_config: dict):
...
SUPPORTED_ALGOS = [
    "PriorityQueueDQN", "PriorityQueueDDQN", "PriorityQueueDueling", 
    "PriorityQueuePerDQN", "PriorityQueueRainbow", "PriorityQueueMultiStepDQN",
    "PriorityQueueFCFS", "PriorityQueueGreedy", "PriorityQueueFORLAPS"
]

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("❌ Missing algorithms")
        print("   Example: python main_3_priority_queue.py PriorityQueueDQN 200")
    ...
    # Baseline algorithms
    if ALGO_TO_RUN in ["PriorityQueueFCFS", "PriorityQueueGreedy", "PriorityQueueFORLAPS"]:
        ...
        baseline_metrics = run_priority_queue_simulation(NUM_PATIENTS, None, "random_base_eval", EVAL_SEED, "Random", 0)
        baseline_avg_time = baseline_metrics['overall_avg_time']
        _append_note(log_dir, f"Random base Avg Time: {baseline_avg_time:.2f}\n", NUM_PATIENTS)
        
        metrics = run_priority_queue_simulation(NUM_PATIENTS, agent, f"priorityqueue_{ALGO_TO_RUN.lower()}", EVAL_SEED, ALGO_TO_RUN, 0)
    ...
    if os.path.exists(random_base_queue_log):
        print(f"✅ Found existing priority queue random_base")
        baseline_metrics = run_priority_queue_simulation(NUM_PATIENTS, None, "random_base_eval", EVAL_SEED, "Random", 0)
    else:
        print(f"⚠️ Priority queue random_base not found. Generating...")
        baseline_metrics = run_priority_queue_simulation(NUM_PATIENTS, None, "random_base", EVAL_SEED, "Random", 0)
    ...
    for gen_id in range(1, 4):
        ...
        ckpt_paths = train_priority_queue_generic(agent, ALGO_TO_RUN, NUM_PATIENTS, gen_id, train_config)
        ...
        for ckpt_path in ckpt_paths:
            ...
            metrics = run_priority_queue_simulation(NUM_PATIENTS, agent_eval, f"priorityqueue_g{gen_id}_ep{ep}", EVAL_SEED, ALGO_TO_RUN, gen_id)
            avg_time = metrics['overall_avg_time']
            improvement = ((baseline_avg_time - avg_time) / baseline_avg_time) * 100 if baseline_avg_time > 0 else 0.0
        ...
        if best_ckpt:
            ...
            if gen_id < 3 and (gen_id + 1) in train_config:
                ...
                run_priority_queue_simulation(NUM_PATIENTS, agent_for_data, f"priorityqueue_gen_{gen_id}", EVAL_SEED, ALGO_TO_RUN, gen_id)
