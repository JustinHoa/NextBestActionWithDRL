...
class RandomDynamicQueueHealthCenter:
    """Health center with Random Dynamic Queue: capacity uniformly sampled in [1×, 3×]."""
    ...
    def __init__(self, env):
        ...
        self.current_max_queue = self._sample_random_capacity()
        
        self.finished_patient_count = 0
    
    def _sample_random_capacity(self):
        """Sample capacity uniformly from [1×, 3×] for concurrent activities."""
        capacity = {}
        for name, data in activity_info.items():
            idx = name_to_id[name] - 1
            if idx in CLUSTER_1_INDICES or idx in CLUSTER_2_INDICES:
                # Concurrent activities: Random
                min_cap = 1.0 * data["staff"]
                max_cap = 3.0 * data["staff"]
                capacity[name] = np.random.uniform(min_cap, max_cap)
            else:
...
class RandomDynamicQueueCoordinator:
    ...
    def predict(self, patient, current_queues_arr, current_time, candidates_names: list, center):
        """Predict next action with random capacity."""
        ...
        # If no available candidates, choose randomly (shouldn't happen often)
...
class RandomDynamicQueuePatient:
    ...
    def go_process(self, coordinator):
        ...
        # Cluster 1 with random capacity
        while c1:
        ...
        # Cluster 2 with random capacity
        while c2:
            q = self.center.get_queue_status_array()
...
def run_random_dynamic_queue_simulation(num_patients=200, agent=None, version_output="random_dynamic_queue", seed=None, model_name="RandomDynamicQueueDQN", gen_id=0):
    """Run simulation with random dynamic queue constraints."""
    ...
    center = RandomDynamicQueueHealthCenter(env)
    coord = RandomDynamicQueueCoordinator(agent, total_sim_time=1440)
    ...
    def patient_gen(patient_list_ref):
        for i in range(num_patients):
            p = RandomDynamicQueuePatient(env, center, i, random.choice(["Male", "Female"]), random.choice(["Single", "Married"]), event_logs)
    ...
    print(f"✅ Random Dynamic Queue Simulation finished.")
