"""
Main script for Dynamic Queue (Naive) training and evaluation.
Supports: DynamicQueueDQN, DynamicQueueDDQN, DynamicQueueDueling, DynamicQueuePerDQN, DynamicQueueRainbow, DynamicQueueMultiStepDQN
...
from agents.dynamic_queue_fcfs_agent import DynamicQueueFCFSAgent
from agents.dynamic_queue_greedy_agent import DynamicQueueGreedyAgent
...
def get_agent(algo_name: str, seed: int = 0):
    if algo_name == "DynamicQueueDQN":
        return DynamicQueueDQNAgent(STATE_SIZE_DYNAMIC_QUEUE, ACTION_SIZE_DYNAMIC_QUEUE, seed=seed)
    if algo_name == "DynamicQueueDDQN":
        return DynamicQueueDDQNAgent(STATE_SIZE_DYNAMIC_QUEUE, ACTION_SIZE_DYNAMIC_QUEUE, seed=seed)
    if algo_name == "DynamicQueueDueling":
        return DynamicQueueDuelingAgent(STATE_SIZE_DYNAMIC_QUEUE, ACTION_SIZE_DYNAMIC_QUEUE, seed=seed)
    if algo_name == "DynamicQueuePerDQN":
        return DynamicQueuePerAgent(STATE_SIZE_DYNAMIC_QUEUE, ACTION_SIZE_DYNAMIC_QUEUE, seed=seed)
    if algo_name == "DynamicQueueRainbow":
        return DynamicQueueRainbowAgent(STATE_SIZE_DYNAMIC_QUEUE, ACTION_SIZE_DYNAMIC_QUEUE, seed=seed)
    if algo_name == "DynamicQueueMultiStepDQN":
        return DynamicQueueMultiStepAgent(STATE_SIZE_DYNAMIC_QUEUE, ACTION_SIZE_DYNAMIC_QUEUE, seed=seed)
    if algo_name == "DynamicQueueFCFS":
        return DynamicQueueFCFSAgent(STATE_SIZE_DYNAMIC_QUEUE, ACTION_SIZE_DYNAMIC_QUEUE, seed=seed)
    if algo_name == "DynamicQueueGreedy":
        return DynamicQueueGreedyAgent(STATE_SIZE_DYNAMIC_QUEUE, ACTION_SIZE_DYNAMIC_QUEUE, seed=seed)
    if algo_name == "DynamicQueueFORLAPS":
...
def train_dynamic_queue_generic(agent, algo_name: str, num_patients: int, gen_id: int, train_config: dict):
...
SUPPORTED_ALGOS = [
    "DynamicQueueDQN", "DynamicQueueDDQN", "DynamicQueueDueling", 
    "DynamicQueuePerDQN", "DynamicQueueRainbow", "DynamicQueueMultiStepDQN",
    "DynamicQueueFCFS", "DynamicQueueGreedy", "DynamicQueueFORLAPS"
]

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("❌ Missing algorithms")
        print("   Example: python main_4_dynamic_queue.py DynamicQueueDQN 200")
    ...
    # Baseline algorithms
    if ALGO_TO_RUN in ["DynamicQueueFCFS", "DynamicQueueGreedy", "DynamicQueueFORLAPS"]:
        ...
        baseline_avg_time = run_dynamic_queue_simulation(NUM_PATIENTS, None, "random_base_eval", EVAL_SEED, "Random", 0)
        _append_note(log_dir, f"Random base Avg Time: {baseline_avg_time:.2f}\n", NUM_PATIENTS)
        
        avg_time = run_dynamic_queue_simulation(NUM_PATIENTS, agent, f"dynamicqueue_{ALGO_TO_RUN.lower()}", EVAL_SEED, ALGO_TO_RUN, 0)
    ...
    if os.path.exists(random_base_queue_log):
        print(f"✅ Found existing dynamic queue random_base")
        baseline_avg_time = run_dynamic_queue_simulation(NUM_PATIENTS, None, "random_base_eval", EVAL_SEED, "Random", 0)
    else:
        print(f"⚠️ Dynamic queue random_base not found. Generating...")
        baseline_avg_time = run_dynamic_queue_simulation(NUM_PATIENTS, None, "random_base", EVAL_SEED, "Random", 0)
    ...
    for gen_id in range(1, 4):
        ...
        ckpt_paths = train_dynamic_queue_generic(agent, ALGO_TO_RUN, NUM_PATIENTS, gen_id, train_config)
        ...
        for ckpt_path in ckpt_paths:
            ...
            avg_time = run_dynamic_queue_simulation(NUM_PATIENTS, agent_eval, f"dynamicqueue_g{gen_id}_ep{ep}", EVAL_SEED, ALGO_TO_RUN, gen_id)
            improvement = ((baseline_avg_time - avg_time) / baseline_avg_time) * 100 if baseline_avg_time > 0 else 0.0
        ...
        if best_ckpt:
            ...
            if gen_id < 3 and (gen_id + 1) in train_config:
                ...
                run_dynamic_queue_simulation(NUM_PATIENTS, agent_for_data, f"dynamicqueue_gen_{gen_id}", EVAL_SEED, ALGO_TO_RUN, gen_id)
