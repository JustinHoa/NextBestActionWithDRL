"""
Main script for Static Queue training and evaluation.
Supports: StaticQueueDQN, StaticQueueDDQN, StaticQueueDueling, StaticQueuePerDQN, StaticQueueRainbow, StaticQueueMultiStepDQN
...
from agents.static_queue_fcfs_agent import StaticQueueFCFSAgent
from agents.static_queue_greedy_agent import StaticQueueGreedyAgent
...
def get_agent(algo_name: str, seed: int = 0):
    if algo_name == "StaticQueueDQN":
        return StaticQueueDQNAgent(STATE_SIZE_STATIC_QUEUE, ACTION_SIZE_STATIC_QUEUE, seed=seed)
    if algo_name == "StaticQueueDDQN":
        return StaticQueueDDQNAgent(STATE_SIZE_STATIC_QUEUE, ACTION_SIZE_STATIC_QUEUE, seed=seed)
    if algo_name == "StaticQueueDueling":
        return StaticQueueDuelingAgent(STATE_SIZE_STATIC_QUEUE, ACTION_SIZE_STATIC_QUEUE, seed=seed)
    if algo_name == "StaticQueuePerDQN":
        return StaticQueuePerAgent(STATE_SIZE_STATIC_QUEUE, ACTION_SIZE_STATIC_QUEUE, seed=seed)
    if algo_name == "StaticQueueRainbow":
        return StaticQueueRainbowAgent(STATE_SIZE_STATIC_QUEUE, ACTION_SIZE_STATIC_QUEUE, seed=seed)
    if algo_name == "StaticQueueMultiStepDQN":
        return StaticQueueMultiStepAgent(STATE_SIZE_STATIC_QUEUE, ACTION_SIZE_STATIC_QUEUE, seed=seed)
    if algo_name == "StaticQueueFCFS":
        return StaticQueueFCFSAgent(STATE_SIZE_STATIC_QUEUE, ACTION_SIZE_STATIC_QUEUE, seed=seed)
    if algo_name == "StaticQueueGreedy":
        return StaticQueueGreedyAgent(STATE_SIZE_STATIC_QUEUE, ACTION_SIZE_STATIC_QUEUE, seed=seed)
    if algo_name == "StaticQueueFORLAPS":
        return DQNAgent(STATE_SIZE_STATIC_QUEUE, ACTION_SIZE_STATIC_QUEUE, seed=seed)
...
def train_static_queue_dqn(agent, num_patients: int, gen_id: int, train_config: dict):
    log_dir = os.path.join("logs", "StaticQueueDQN")
    ensure_dir(log_dir)
    ...
    plot_training_status(all_scores, all_losses, "StaticQueueDQN", gen_id, save_dir=log_dir, num_patients=num_patients)
...
SUPPORTED_ALGOS = [
    "StaticQueueDQN", "StaticQueueDDQN", "StaticQueueDueling", 
    "StaticQueuePerDQN", "StaticQueueRainbow", "StaticQueueMultiStepDQN",
    "StaticQueueFCFS", "StaticQueueGreedy", "StaticQueueFORLAPS"
]

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("❌ Missing algorithms")
        print("   Example: python main_2_static_queue.py StaticQueueDQN 200")
    ...
    # Baseline algorithms
    if ALGO_TO_RUN in ["StaticQueueFCFS", "StaticQueueGreedy", "StaticQueueFORLAPS"]:
        ...
        baseline_avg_time = run_static_queue_simulation(NUM_PATIENTS, None, "random_base_eval", EVAL_SEED, "Random", 0)
        _append_note(log_dir, f"Random base Avg Time: {baseline_avg_time:.2f}\n", NUM_PATIENTS)
        
        avg_time = run_static_queue_simulation(NUM_PATIENTS, agent, f"staticqueue_{ALGO_TO_RUN.lower()}", EVAL_SEED, ALGO_TO_RUN, 0)
    ...
    if os.path.exists(random_base_queue_log):
        print(f"✅ Found existing static queue random_base")
        baseline_avg_time = run_static_queue_simulation(NUM_PATIENTS, None, "random_base_eval", EVAL_SEED, "Random", 0)
    else:
        print(f"⚠️ Static queue random_base not found. Generating...")
        baseline_avg_time = run_static_queue_simulation(NUM_PATIENTS, None, "random_base", EVAL_SEED, "Random", 0)
    ...
    for gen_id in range(1, 4):
        ...
        for ckpt_path in ckpt_paths:
            ...
            avg_time = run_static_queue_simulation(NUM_PATIENTS, agent_eval, f"staticqueue_g{gen_id}_ep{ep}", EVAL_SEED, ALGO_TO_RUN, gen_id)
            improvement = ((baseline_avg_time - avg_time) / baseline_avg_time) * 100 if baseline_avg_time > 0 else 0.0
        ...
        if best_ckpt:
            ...
            if gen_id < 3 and (gen_id + 1) in train_config:
                ...
                run_static_queue_simulation(NUM_PATIENTS, agent_for_data, f"staticqueue_gen_{gen_id}", EVAL_SEED, ALGO_TO_RUN, gen_id)
