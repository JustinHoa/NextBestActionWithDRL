...
class TabularSARSAAgent:
    """On-policy tabular SARSA (Hundogan et al., 2025).

    State key: 21-bit bitmask of completed activities (prefix).
    Uses the actual next action from the episode (on-policy update).
    ...
    def update(self, s_key: int, a: int, r: float,
               s_next_key: int, a_next: int, done: bool) -> None:
        td_target = r if done else r + self.gamma * self.q_table[s_next_key][a_next]
