...
class RandomDynamicQueueEnv:
    """
    Environment with Random Dynamic Queue Constraints:
    - Capacity for each concurrent activity is randomly sampled
    - Range: [1 × resource, 3 × resource] uniform distribution
    ...
    def __init__(self, state_size, action_size, data_path, activity_info_path='data/raw/activity_info.json'):
        ...
        # Load Trace Data
        print(f"📂 RandomDynamicQueueEnv loading data: {data_path}")
        try:
        ...
        # Random Dynamic Queue: capacity uniformly sampled in [1×, 3×]
    ...
    def _sample_random_capacity(self):
        """
        Sample capacity uniformly from [1×resource, 3×resource] for concurrent activities.
        ...
        for i in range(21):
            if i in self.concurrent_indices:
                # Sample uniformly from [1×resource, 3×resource]
                min_cap = self.min_multiplier * self.staff_arr[i]
                max_cap = self.max_multiplier * self.staff_arr[i]
                capacity[i] = np.random.uniform(min_cap, max_cap)
            else:
    ...
    def reset(self):
        ...
        # Sample random capacity for this episode
    ...
    def get_action_mask(self):
        """
        Action mask with random capacity (fixed per episode).
        """
        ...
        # Queue capacity constraints (randomly sampled capacity)
        for i in range(21):
